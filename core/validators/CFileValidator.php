...
class CFileValidator extends CValidator
{
	...
	protected function validateAttribute($object, $attribute)
	{
		if($this->maxFiles > 1)
		{
			...
			{
				$message=$this->tooMany!==null?$this->tooMany : Yii::t('app', '{attribute} cannot accept more than {limit} files.');
	...
	protected function validateFile($object, $attribute, $file)
	{
		...
		{
			$message=$this->tooLarge!==null?$this->tooLarge : Yii::t('app','The file "{file}" is too large. Its size cannot exceed {limit} bytes.');
			$this->addError($object,$attribute,$message,array('{file}'=>$file->getName(), '{limit}'=>$this->getSizeLimit()));
		}
		else if($error==UPLOAD_ERR_PARTIAL)
			throw new CException(Yii::t('app','The file "{file}" was only partially uploaded.',array('{file}'=>$file->getName())));
		else if($error==UPLOAD_ERR_NO_TMP_DIR)
			throw new CException(Yii::t('app','Missing the temporary folder to store the uploaded file "{file}".',array('{file}'=>$file->getName())));
		else if($error==UPLOAD_ERR_CANT_WRITE)
			throw new CException(Yii::t('app','Failed to write the uploaded file "{file}" to disk.',array('{file}'=>$file->getName())));
		else if(defined('UPLOAD_ERR_EXTENSION') && $error==UPLOAD_ERR_EXTENSION)  // available for PHP 5.2.0 or above
			throw new CException(Yii::t('app','File upload was stopped by extension.'));

		if($this->minSize!==null && $file->getSize()<$this->minSize)
		{
			$message=$this->tooSmall!==null?$this->tooSmall : Yii::t('app','The file "{file}" is too small. Its size cannot be smaller than {limit} bytes.');
		...
		if($this->types!==null)
		{
			...
			{
				$message=$this->wrongType!==null?$this->wrongType : Yii::t('app','The file "{file}" cannot be uploaded. Only files with these extensions are allowed: {extensions}.');
