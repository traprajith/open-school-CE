...
class AchievementsController extends RController
{
		...
		{ 
			...
			if($model->user_id!='' and $model->doc_title!='' and $model->file!='' and $model->file_type!='') // Checking if Document name and file is uploaded
				{
				  if(in_array($model->file_type,$valid_file_types)) // Checking file type
					...
					if($file_size <= 5242880) // Checking file size
					{
						...
						if(!is_dir('uploadedfiles/employee_employee_achievement_document/')) // Creating student_document directory
						{
							mkdir('uploadedfiles/employee_achievement_document/');
						}
						if(!is_dir('uploadedfiles/employee_achievement_document/'.$model->user_id)) // Creating student directory for saving the files
						{
							mkdir('uploadedfiles/employee_achievement_document/'.$model->user_id);
						}
						
						$temp_file_loc = $_FILES['Achievements']['tmp_name']['file'];
						$destination_file = 'uploadedfiles/employee_achievement_document/'.$model->user_id.'/'.$file_name;
						if(move_uploaded_file($temp_file_loc,$destination_file)) // Saving the files to the folder
						{
							...
							{
								if($obj_img!=NULL){	
									DocumentUploads::model()->insertData(6, $model->id, $file_name, 2);	
									}
			...
			if($flag == 1) // If no errors, go to next step of the student registration
			{
				...
				//$this->redirect(array('view','id'=>$model->id));
					...
					$this->redirect(array('employees/achievements','id'=>$_POST['Achievements']['eid']));
	...
	public function actionDeletes()
	{
		...
		$destination_file = 'uploadedfiles/employee_achievement_document/'.$model->user_id.'/'.$model->file;
		...
		//var_dump($_REQUEST['employee_id']);exit;
	...
	public function actionUpdate()
	{
		   ...
		   $obj_img			=	 CUploadedFile::getInstance($model,'file');
			...
			{ 
				...
				{ 
					if($obj_img!=NULL)
					{	
						DocumentUploads::model()->insertData(6, $model->id, $file_name, 2, $old_file_name);	
					}
					if($_FILES['Achievements']['name']['file']!=NULL)
					{
						$old_destination_file = 'uploadedfiles/employee_achievement_document/'.$model->user_id.'/'.$old_file_name;	
				...
				else
				{
					...
					$this->redirect(array('update','id'=>$model->id,'employee_id'=>$_POST['Achievements']['eid']));
				}
		...
		$this->render('update',array(
			'model'=>$model,'employee_id'=>$_REQUEST['employee_id']
		));
