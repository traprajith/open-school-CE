<?php if( $items['controllers']!==array() ): ?>
	...
	<?php foreach( $items['controllers'] as $key=>$item ): ?>
		...
		<?php if( isset($item['actions'])===true && $item['actions']!==array() ): ?>
			...
			<tr class="augen-row <?php echo $controllerExists===true ? 'augen' : ''; ?>">
				<td><?php echo $controllerExists===false ? $form->checkBox($model, 'items['.$controllerKey.'.*]') : ''; ?></td>
				<td ><?php echo ucfirst($item['name']).'.*'; ?></td>
				<td ><?php echo substr($item['path'], $basePathLength+1); ?></td>
			</tr>
			...
			<?php $i=0; foreach( $item['actions'] as $action ): ?>
				...
				<tr class="action-row<?php echo $actionExists===true ? ' augen' : ''; ?><?php echo ($i++ % 2)===0 ? ' odd' : ' even'; ?>">
					<td><?php echo $actionExists===false ? $form->checkBox($model, 'items['.$actionKey.']') : ''; ?></td>
					<td><?php echo $action['name']; ?></td>
					<td ><?php echo substr($item['path'], $basePathLength+1).(isset($action['line'])===true?':'.$action['line']:''); ?></td>
				</tr>
