...
.del_msg{ color:#962326; font-weight:bold; padding-top:5px; text-align:center; }
</style>
<?php 
/**
                    ...
                    <div class="emp_right_contner">
                        ...
                        <div class="clear"></div>
                            <div class="emp_cntntbx" style="padding-top:10px;">                            
                                <div>
                                    <div> 
                                        <h3><?php echo Yii::t('app','Elective Groups'); ?></h3>
...
<div class="top-hed-btn-right">
                                        	<ul>
												...
												{
													$template = $template.'{elective-groups_update}';
												}
												
												if(($year == $current_academic_yr->config_value) or ($year != $current_academic_yr->config_value and $is_delete->settings_value!=0))
												{
													$template = $template.'{elective-groups_delete}';
												}
                                                ...
                                                ?>
                                                <li>
                                                     <?php echo CHtml::link(Yii::t('app','Create Elective Groups'), array('#'),array('id'=>'add_electives','class'=>'a_tag-btn'));?>     
                                                </li>
                                                <?php
                                                }
                                                ?>
                                                <li>
                                                	<?php echo CHtml::link(Yii::t('app','Electives'), array('/courses/electives','id'=>$_REQUEST['id']),array('class'=>'a_tag-btn'));?>
                                                </li>
											</ul>
										</div>
                                        </div>
                                        ...
                                        $del_con = Yii::t('app', 'Are you sure you want to delete this elective group(Also delete this electives related exam Scheduled and Exam marks)?');
                                        ...
                                        'template' => '{elective-groups_view}'.$template,
                                        ),
                                        ),
    ...
    $('.fan_del').each(function(index) {
        ...
        deletes[id] = function() {
            $.ajax({
                ...
                success: function(data) {					
					$(".del_msg").html("<?php echo Yii::t('app','Selected elective group deleted successfully!'); ?>");
					$(".del_msg").fadeOut(6000);
                    var res = jQuery.parseJSON(data);
                     var page=$("li.selected  > a").text();
                    $.fn.yiiGridView.update('elective-groups-grid', {url:'<?php echo Yii::app()->request->getUrl()?>',data:{"ElectiveGroups_page":page}});
                }//success
            });//ajax
        };//end of deletes
...
//CREATE 
    ...
    $('#add_electivesubjects ').bind('click', function() {
        $.ajax({
            type: "POST",
