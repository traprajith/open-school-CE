...
class ElectiveGroupsController extends RController
{
	...
	public function actionReturnForm(){	
		...
		if(isset($_POST['update_id'])){
			...
			//max weekly classes
			if($model!=NULL){
				$subject	= Subjects::model()->findbyAttributes(array('elective_group_id'=>$model->id,'batch_id'=>$model->batch_id));
				if($subject!=NULL){
					$model->max_weekly_classes	= $subject->max_weekly_classes;
				}
			}
		}
		else 
		...
		if(isset($_POST['ElectiveGroups'])){
			...
			if( $model->save(false)){
				...
				if($subject!=NULL){
					...
					$subject->max_weekly_classes 	= $model->max_weekly_classes;
					$subject->save();
				}				
	...
	public function actionAjax_delete(){
		...
		if ($deleted->delete() ){	
			...
			CbscExams::model()->deleteAllByAttributes(array('subject_id'=>$subject->id));
