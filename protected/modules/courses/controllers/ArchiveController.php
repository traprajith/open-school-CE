					...
					if($is_email_exist == NULL and $is_phone_exist == NULL){
						if($model->saveAttributes(array('is_active'=>1, 'is_deleted'=>0))){
							if($model->uid != 0){
								...
								Students::model()->createStudentUser($model->id); //Create student user
							}
			...
			if(count($id) > 0){
				for($i = 0; $i < count($id); $i++){					
					...
					if($model->delete()){
						...
						//Delete Exam scores
						$examscores = ExamScores::model()->findAllByAttributes(array('student_id'=>$id[$i]));
						if($examscores){
					...
					if($is_email_exist == NULL and $is_phone_exist == NULL){
						if($model->saveAttributes(array('is_delete'=>0))){
							if($model->uid != 0){
								...
								Guardians::model()->createGuardianUser($model->id); //Create Guardian User
							}
							$restore_success = $restore_success + 1;
						}
					}
