...
class SubjectNameAjaxController extends RController
{
	/**
...
public function   init() {
  ...
  private function registerAssets(){
	...
	{
		$model=SubjectNameAjax::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='subject-name-ajax-form')
		{
        ...
        $this->renderPartial('view',array('model'=>$model),false, true);
             ...
             if(isset($_POST['update_id']))$model= $this->loadModel($_POST['update_id']);else $model=new SubjectNameAjax;
		...
		$model=new SubjectNameAjax('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['SubjectNameAjax']))
			$model->attributes=$_GET['SubjectNameAjax'];
		...
		if(isset($_POST['SubjectNameAjax']))
		{
           $model=$this->loadModel($_POST['update_id']);
			$model->attributes=$_POST['SubjectNameAjax'];
			if( $model->save(false)){
