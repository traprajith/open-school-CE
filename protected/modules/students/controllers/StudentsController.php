...
class StudentsController extends RController
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','manage','Website','savesearch','events','DisplaySavedImage','Pdf','Printpdf','Remove','Search','inactive','active','deletes','Document','electives','removeelective','Courses','Flag'),
				'users'=>array('@'),
			),
	 ...
	 */
	 public function actionManage()
	 {
		...
		if( isset($_REQUEST['cid']) and $_REQUEST['cid']!=NULL and (isset($_REQUEST['Students']['batch_id']) and $_REQUEST['Students']['batch_id']!=NULL and $_REQUEST['Students']['batch_id']!=0) or (isset($_REQUEST['rollnumber']) and $_REQUEST['rollnumber']!=NULL))
		{
			...
			if($_REQUEST['Students']['batch_id'] != NULL and $_REQUEST['rollnumber'] != NULL){
				...
				$criteria->condition	= $criteria->condition.' and (`t1`.`result_status`=:result_status and `t1`.`batch_id`=:batch_id and `t1`.`roll_no`=:roll_no)';
				...
				$criteria->join			= 'JOIN `batch_students` `t1` ON `t`.`id` = `t1`.`student_id`'; 
				$criteria->condition	= $criteria->condition.' and (`t1`.`result_status`=:result_status and `t1`.`batch_id`=:batch_id)';
				//$criteria->params[':batch_status']	= 1;
				$criteria->params[':result_status']	= 0;
				$criteria->params[':batch_id']		= $_REQUEST['Students']['batch_id'];								
			}
				...
				$criteria->condition	= $criteria->condition.' and (`t1`.`result_status`=:result_status and `t1`.`roll_no`=:roll_no and `t2`.`is_active`=:is_active and `t2`.`is_deleted`=:is_deleted)';
				//$criteria->params[':batch_status']	= 1;
				$criteria->params[':result_status']	= 0;				
				$criteria->params[':roll_no']		= $_REQUEST['rollnumber'];
		...
		{ 
			...
			$model->academic_yr = $_REQUEST['Students']['academic_yr'];
			$batch_stu = BatchStudents::model()->findAllByAttributes(array('result_status'=>0,'academic_yr_id'=>$year));
			$students	=array();
