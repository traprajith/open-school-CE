...
<div class="right">
    <input id="add_events" type="button" style="display:block; clear: both;"
           value="Create Events" class="client-val-form button">
</div>
 ...
 //VIEW
    ...
    $('.fan_view').each(function(index) {
        ...
        $(this).bind('click', function() {
            $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->request->baseUrl;?>/index.php?r=/events/returnView",
                data:{"id":id,"YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken;?>"},
                beforeSend : function() {
                    $("#events-grid").addClass("ajax-sending");
                },
                complete : function() {
                    $("#events-grid").removeClass("ajax-sending");
                },
...
//UPDATE
    ...
    $('.fan_update').each(function(index) {
        ...
        $(this).bind('click', function() {
            $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->request->baseUrl;?>/index.php?r=/events/returnForm",
                ...
                success: function(data) {
                    $.fancybox(data,
                            {    "transitionIn"    :  "elastic",
                                ...
                                $.fn.yiiGridView.update('events-grid', {url:'<?php echo Yii::app()->request->getUrl()?>',data:{"Events_page":page}});
                                }//onclosed
                            });//fancybox
                    //  console.log(data);
                } //success
...
// DELETE
    ...
    $('.fan_del').each(function(index) {
        ...
        deletes[id] = function() {
            $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->request->baseUrl;?>/index.php?r=/events/ajax_delete",
                data:{"id":id,"YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken;?>"},
                    beforeSend : function() {
                    $("#events-grid").addClass("ajax-sending");
                },
...
//CREATE 
    ...
    $('#add_events ').bind('click', function() {
        $.ajax({
            type: "POST",
            url: "<?php echo Yii::app()->request->baseUrl;?>/index.php?r=/events/returnForm",
