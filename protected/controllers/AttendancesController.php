...
class AttendancesController extends RController
{
	/**
...
public function   init() {
  ...
  private function registerAssets(){
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('ajax_create','ajax_update','Attentance','Addnew'),
				'users'=>array('@'),
			),
	...
	{
		$model=Attendances::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='attendances-form')
		{
        ...
        $this->renderPartial('view',array('model'=>$model),false, true);
             ...
             if(isset($_POST['update_id']))$model= $this->loadModel($_POST['update_id']);else $model=new Attendances;
		...
		$model=new Attendances('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Attendances']))
			$model->attributes=$_GET['Attendances'];
	...
	public function actionAttentance()
	{
		...
		$this->render('attentance',array('model'=>$model));
	}
		...
		if(isset($_POST['Attendances']))
		{
           $model=$this->loadModel($_POST['update_id']);
			$model->attributes=$_POST['Attendances'];
			if( $model->save(false)){
        ...
        {       $flag=false;
            ...
            if($model->save()) {
                ...
                if($flag) {
                    ...
                    $this->renderPartial('create',array('model'=>$model,'day'=>$_GET['day'],'month'=>$_GET['month'],'year'=>$_GET['year'],'emp_id'=>$_GET['emp_id'],'period'=>$_GET['period']),false,true);
                }
        }
